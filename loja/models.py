...
class Produto(models.Model):
    ...
    detalhe = models.CharField(max_length=200,null=True, blank=True)

    def __str__(self):
        return f"Nome: {self.descricao}, Categoria :{self.categoria}, Tipo: {self.tipo}, Preço: {self.preco}"
    
    def qtdetotalvendas(self):
        itens = ItensPedido.objects.filter(pedido__finalizado=True, item_estoque__produto=self.id)
        qtdevendas = sum([item.quantidade for item in itens])
        return qtdevendas
...
class ItemEstoque(models.Model):
    ...
    def __str__(self) -> str:
        return f"{self.produto.descricao}, Tamanho:{self.tamanho}, Cor:{self.cor.descricao}"
...
class Endereco(models.Model):
    ...
    def __str__(self) -> str:
        return f"Cliente: {self.cliente.email} - Rua: {self.rua} - Cidade: {self.cidade} - Estado: {self.estado} - CEP: {self.cep}"
...
class ItensPedido(models.Model):
    ...
    def __str__(self) -> str:
        return f"ID pedido: {self.pedido.id} - Produto: {self.item_estoque.produto.descricao}, {self.item_estoque.tamanho}, {self.item_estoque.cor.descricao} "
...
class Banner(models.Model):
    ...
    def __str__(self):
        return f"Banner: {self.link_destino}, Ativo :{self.ativo}"
